...
class Api(github: Github)(implicit val ec: ExecutionContext) {
        ...
        r =>
          (
              Pagination(
                  current = clampedPage,
                  totalPages =
                    Math.ceil(r.totalHits / resultsPerPage.toDouble).toInt,
                  total = r.totalHits
              ),
  ...
  def projectPage(projectRef: Project.Reference,
                  selectedArtifact: Option[String] = None,
                  selectedVersion: Option[SemanticVersion] = None)
    : Future[Option[(Project,
                     List[String],
                     List[SemanticVersion],
                     List[(ScalaTarget, MavenReference)],
                     Release,
                     Int)]] = {
    ...
    def defaultRelease(project: Project, releases: List[Release])
      : Option[(Release,
                List[SemanticVersion],
                List[(ScalaTarget, MavenReference)])] = {
      val artifacts = releases.groupBy(_.reference.artifact).toList

      def specified(artifact: String): Boolean =
        Some(artifact) == selectedArtifact
      def projectDefault(artifact: String): Boolean =
        Some(artifact) == project.defaultArtifact
      def projectRepository(artifact: String): Boolean =
          ...
          case Some(a) => {
            ...
            ArtifactNameParser(a) match {
              case Some((name, target)) => {
                artifacts.find { case (a, b) => a == name }.map {
                  case (a, b)                => (a, b, Some(target))
                }
              }
              case None =>
                finds(artifacts, List(specified _)).map {
                  case (a, b) => (a, b, None)
                }
            }
          }
      ...
      artifact.flatMap {
        case (_, releases, selectedTarget) =>
          val sortedByVersion =
            releases.sortBy(_.reference.version)(Descending)

          // version provided by the user
          val versionSelected = selectedVersion
            .map(version =>
                  sortedByVersion.filter(_.reference.version == version))
          ...
          val latestStableTarget = {
            selectedTarget match {
              case None => {
                ...
                val scalaJsOut =
                  if (versionSelected.exists(
                          _.reference.target.scalaJsVersion.isEmpty)) {
                    versionSelected.filter(
                        _.reference.target.scalaJsVersion.isEmpty)
                  } else versionSelected

                val preReleaseOut =
                  if (scalaJsOut.exists(
                          _.reference.target.scalaVersion.preRelease.isEmpty)) {
                    scalaJsOut.filter(
                        _.reference.target.scalaVersion.preRelease.isEmpty)
                  } else scalaJsOut

                preReleaseOut
                  .sortBy(_.reference.target.scalaVersion)(Descending)
              }
              case Some(target) =>
          ...
          val lastNonPreReleases = {
            ...
            val nonPreRelease =
              if (latestStableTarget.exists(
                      _.reference.version.preRelease.isEmpty)) {
                latestStableTarget.filter(
                    _.reference.version.preRelease.isEmpty)
              } else {
                latestStableTarget
              }
          ...
          lastNonPreReleases.headOption.map(
              r =>
                (
                    r,
    ...
    projectAndReleases.map {
      case (p, releases) =>
        p.flatMap(project =>
              defaultRelease(project, releases).map {
            case (release, versions, targets) =>
              val artifacts = releases
                .groupBy(_.reference.artifactReference)
                .keys
                .toList
                .map(_.artifact)
