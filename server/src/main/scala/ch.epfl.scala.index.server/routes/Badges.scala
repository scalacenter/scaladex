...
class Badges(dataRepository: DataRepository) {
  ...
  private def shieldsSvg(rawSubject: String, rawStatus: String, rawColor: Option[String],
    ...
    get {
      path(Segment / Segment / Segment / "latest.svg") { (organization, repository, artifact) =>
        shields { (color, style, logo, logoWidth) =>
          onSuccess(dataRepository.projectPage(Project.Reference(organization, repository), 
