  ...
  val routes =
    get(
      concat(
        ...
        path("assets" / "img" / Remaining)(
          path => getFromResource("img/" + path)
        ),
        path("assets" / "css" / Remaining)(
          path => getFromResource("css/" + path)
        ),
        path("assets" / "js" / Remaining)(
          path => getFromResource("js/" + path)
        ),
