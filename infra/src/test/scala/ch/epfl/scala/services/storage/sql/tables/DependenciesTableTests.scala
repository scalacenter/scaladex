...
class DependenciesTableTests
  ...
  describe("should generate the query for") {
    ...
    it("find") {
      ...
      q.sql shouldBe
        s"""|SELECT source_groupId, source_artifactId, source_version, target_groupId,
            ...
            | FROM dependencies WHERE source_groupId=? AND source_artifactId=? AND source_version=?""".stripMargin
          .filterNot(_ == '\n')
    }
    ...
    it("getProjectWithDependentUponProjects") {
      val q = getProjectWithDependentUponProjects()
      check(q)
      q.sql shouldBe
        s"""|SELECT DISTINCT t.organization, t.repository, d.organization, d.repository
            | FROM ( dependencies d INNER JOIN releases r ON d.source_groupid = r.groupid AND
            ...
            | d.target_version = t.version GROUP BY t.organization, t.repository, d.organization, d.repository""".stripMargin
          .filterNot(_ == '\n')
    }
  }
}
