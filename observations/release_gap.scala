...
def findDep(poms: List[(MavenModel, List[BintraySearch])], version: String): List[(MavenModel, List[BintraySearch])] = {
...
implicit val dateOrdering = Ordering.by{ date: Date => Date.unapply(date) }

def plot(poms: List[(MavenModel, List[BintraySearch])], version: String): List[(Date, Int)] = 
...
val poms = PomsReader.load().collect { case Success(pomAndMeta) => pomAndMeta }
...
val data = {
  histograms.flatMap{ case (version, histo) => 
    histo.map{ case (date, count) => 
      (date, version, count)
    }
  }
  ...
  .map{ case (date, versionCount) =>
    (date, versions.map(version => 
      versionCount.get(version).getOrElse(0)
    ))
  }
}
...
def printCsv(in: List[(Date, List[Int])]): String = {
  ...
  in.map{ case (Date(year, month), versionsCount) =>
