  ...
  def convertAll(
    ...
    projectsAndReleases.iterator.map {
      case (seed, releases) =>
        ...
        val scalaDependencies = projectDependencies.collect { case d: ScalaDependency => d }
        ...
        val releasesWithDependencies = releases.map { release =>
          release.copy(
            javaDependencies = javaDependenciesByRelease.getOrElse(release.reference, Seq())
          )
        }
