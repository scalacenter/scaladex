...
class SubIndex(filesystem: LocalStorageRepo, database: SqlDatabase)(implicit ec: ExecutionContext) extends LazyLogging {
  def run(): Future[Unit] = {
...
object SubIndex {
  def run(filesystem: LocalStorageRepo, database: SqlDatabase)(implicit ec: ExecutionContext): Future[Unit] = {
