...
class GithubClientImpl(token: Secret, config: HttpClientConfig = HttpClientConfig.default)(using system: ActorSystem)
  ...
  private def getUserOrganizationsPage(
    ...
    get[GraphQLPage[Project.Organization]](request)(using graphqlPageDecoder("data", "user", "organizations")(using
      d => d.downField("login").as[String].map(Project.Organization.apply)
    ))
