...
object ProjectDocument {
  def apply(project: NewProject, releases: Seq[NewRelease], inverseProjectDependencies: Int): ProjectDocument = {
    ...
    ProjectDocument(
      organization,
      repository,
      releases.map(_.artifactName).distinct,
      hasCli,
      created,
      updatedAt = None,
      platforms.map(_.platformType).distinct,
      platforms.flatMap(_.scalaVersion).map(_.family).distinct,
      platforms.collect { case ScalaJs(_, scalaJsV) => scalaJsV }.distinct,
      platforms.collect { case ScalaNative(_, scalaNativeV) => scalaNativeV }.distinct,
      platforms.collect { case SbtPlugin(_, sbtV) => sbtV }.distinct,
